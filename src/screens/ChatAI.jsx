...
const ChatScreen = ({ navigation }) => {
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      // 🔥 CRITICAL FIX
      await clearSession(); // ❗ old session remove
      setHasValidSession(false); // ❗ force new session
    ...
    Alert.alert("Confirm New Number", "Are you sure you want new number?", [
      { text: "No", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          await handleSend(
            formatted,
            true, // retryWithoutSession → new session
            false, // silent → bot response visible
            false
          );
...
//       setShowSignupForm(false);
//       // Directly show number type selection
//       setShowNumberTypeSelection(true);
//     } catch (error) {
//       console.error("Form submission error:", error);
//       Alert.alert("Error", "Failed to submit form. Please try again.");
